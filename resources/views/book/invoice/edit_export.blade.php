...
@section('content')
    <div class="invoiceWrapper" v-cloak>
                ...
                <table cellspacing="0" border="0" style="border-collapse:collapse;" class="topTable">
                    ...
                    <tr>
                        ...
                        <td width="410" class="border-top">
                            <div class="select_box curList">
                                <input class="selectText" :value="invoiceName" v-model="invoiceName" @focus="showUnit">
                    ...
                    <tr>
                        ...
                        <td rowspan="2">
                            ...
                            <div class="wb-content curList" v-show="wxBzAdd">
                                <span>币种:</span>
                    ...
                    <tr>
                        ...
                        <td><input type="text" class="textTitle" v-model="reqParams.gfdw_yhzh"></td>
                    </tr>
                    </tbody>
                </table>
                            ...
                            <div class="curList">
                                ...
                                <div class="select_box">
                                    <ul class="select_ul" v-show="item.ratesTotal">
                                        <li class="" v-for="ratesList in rateOptions" :key="ratesList">
                                        <em>@{{ratesList}}</em>
                                        </li>
                                    </ul>
                                </div>
        ...
        <div class="keep">
            ...
            <a href="javascript:;" class="keepBtn" @click="addNew">新增</a>
            <a href="javascript:;" class="keepBtn1" @click="showKeepafter">保存</a>
            <a href="javascript:;" class="delBtn" @click="deleteInvoice">删除</a>
            <div href="javascript:;" class="importBtn1" @mouseenter="otherShow = true" @mouseleave="otherShow = false">
                <a href="javascript:;">快速收款</a>
                <ul class="invoice-wx" v-show="otherShow">
                    <li><a href="javascript:;" @click="bank_pay" ref="bankPay" data-href="{{ route('fund.index', ['channel_type' => 1, 'type'=>'invoice', 'invoice_id'=> request('id')]) }}" data-title="银行">银行</a></li>
                    <li><a href="javascript:;" @click="cash_pay" ref="cashPay" data-href="{{ route('fund.index', ['channel_type' => 2, 'type'=>'invoice', 'invoice_id'=> request('id')]) }}" data-title="现金">现金</a></li>
                </ul>
            </div>
        </div>
    </div>
            ...
            created: function () {
                ...
                _this.$http.get('{{url('book/invoice/detail')}}?id={{request('id')}}').then(function (response) {
                    if (response.body.status == 1) {
                        ...
                        response.body.data.zsfs = response.body.data.zsfs == '' ? '[]' : response.body.data.zsfs;

                        _this.reqParams.sub_type = response.body.data.sub_type;
                        _this.reqParams.id = response.body.data.id;
                        _this.reqParams.fphm = response.body.data.fphm;
                        _this.reqParams.fpdm = response.body.data.fpdm;
                        _this.reqParams.kplx = response.body.data.kplx;
                        _this.reqParams.dkfs = response.body.data.dkfs;
                        _this.reqParams.fpzs = response.body.data.fpzs;
                        _this.reqParams.kprq = response.body.data.kprq;
                        _this.reqParams.gfdw_id = response.body.data.gfdw_id;
                        _this.invoiceName = response.body.data.gfdw_name;

                        _this.reqParams.gfdw_nsrsbh = response.body.data.gfdw_nsrsbh;
                        _this.reqParams.gfdw_yhzh = response.body.data.gfdw_yhzh;
                        _this.reqParams.gfdw_dzdh = response.body.data.gfdw_dzdh;
                        _this.reqParams.remark = response.body.data.remark;

                        _this.reqParams.zsfs = JSON.parse(response.body.data.zsfs);
                        while (isString(_this.reqParams.zsfs)) {
                            _this.reqParams.zsfs = JSON.parse(_this.reqParams.zsfs);
                        }

                        //console.log(_this.reqParams.zsfs);

                        _this.tablesDate = [];
                        for (var i in response.body.data.items) {
                            _this.tablesDate.push({
                                id: response.body.data.items[i]['id'],
                                name: response.body.data.items[i]['id'],
                                num: response.body.data.items[i]['num'],
                                money: response.body.data.items[i]['money'],
                                hl: '',
                                rate: response.body.data.items[i]['tax_rate'],
                                ticketVal: response.body.data.items[i]['money'],
                                ywMaterial: response.body.data.items[i]['ywlx_name'],
                                ywMaterial_id: response.body.data.items[i]['ywlx_id'],
                                rateMoneyLine: response.body.data.items[i]['tax_money'],
                                //wxmoneyLine: response.body.data.items[i]['money'],
                                ratePriceLine: response.body.data.items[i]['fee_tax_sum'],
                                smVal: response.body.data.items[i]['tax_name'],
                                kpVal: response.body.data.items[i]['kpxm_name'],
                                ggxh: response.body.data.items[i]['ggxh'],
                                dw: response.body.data.items[i]['dw'],
                                ybje: '',
                                nowIndex: 0,
                                ratesTotal: false,
                                yw: false,
            ...
            methods: {
                ...
                save: function (reload) {
                    ...
                    this.$http.post('{{ url('book/invoice/update') }}', this.reqParams).then(function (response) {
                        ...
                        layer.msg(response.body.info, {icon: status, time: 3000, shade: 0.3});
                        setTimeout(function () {
                            ...
                            if (reload == true && response.body.status == 1) {
                                ...
                                location.href = '{{ url('book/invoice/editExport')  }}/' + _this.reqParams.id;
                            }
                        }, 2000);
                    });
                },
                ...
                // 新增按钮点击
                addNew: function () {
                    parent.creatIframe('{{ url('book/invoice/addExport')  }}', '新增销项发票');
                },
