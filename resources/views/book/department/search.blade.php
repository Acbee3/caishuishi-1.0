...
@section('title')部门员工搜索@endsection
...
@section('css')
    ...
    <!--员工模块-->
    <link rel="stylesheet" href="{{asset("css/book/paid/profess.css")}}">
@endsection
        ...
        <div class="officerRight">
            ...
            <div class="officerSearch">
                ...
                <div class="officerRightBtn">
                    ...
                    <a href="javascript:void(0);" class="professDel" @click="changeDepartment" style="background: #0a6999;">调整部门</a>
                    <a href="javascript:void(0);" class="professDel" @click="delSelected">删除</a>
                </div>
            </div>
            <div class="professorSearch">
                <div class="professContent">
                ...
                <div class="search">
                    ...
                    <i class="icon iconfont icon-search" @click="doSearch"></i>
                </div>
            </div>
            <div class="professTableTotal">
                ...
                <div class="professTableCenter">
                    <table>
                        <tbody>
                        <tr v-if="professorTables && professorTables.length" v-for="(item,index) in professorTables" :key="item">
                            ...
                            <td class="width12">
                                <a href="javascript:void(0);" :class="mapClass[item.personnelState]" @click="changeStatus(index,item.employee_name,item.id,item.personnelState,item)" v-text="statusContent[item.personnelState]"></a>
                            </td>
                            <td class="width12">
                    ...
                    <nav class="container_agerant">
                        @if (!empty($data))
                            {{ $data->appends(['st' => 'employee', 'id' => $DepartmentId, 'sv'=>$sv, 'tit'=>$tit])->links() }}
                        @endif
                    </nav>
    ...
    <div class="import_box">
        <div class="cfs-upload">
            <div class="file-upload" style="width:100%;">
                <form id="uploadDataForm" name="cfs_uploadDataForm"  action="" method="post"  enctype="multipart/form-data">
            ...
            <p class="m_top20">员工导入模板：<a href="/static/assets/models/employee_import.xlsx" style="color:red">点击下载</a></p>
        </div>
    </div>
</script>
        ...
        methods: {
            ...
            delSelected: function () {
                ...
                layer.confirm('确定要删除已选择的人员吗？', {icon: 3, title:'提示'},
                    function () {
                        _this.$http.post('{{ route('employee.api_del_ids') }}', {_token:"{{csrf_token()}}",ids:_this.selected}).then(function (response) {
            ...
            professDel: function (index, name, id) {
                ...
                layer.confirm('确定要删除员工 '+name+' 吗？', {icon: 3, title:'提示'},
                    function () {
                        //console.log(index);
                        _this.$http.post('{{ route('employee.api_del') }}', {_token:"{{csrf_token()}}",id:id}).then(function (response) {
                            if (response.body.status == 'success'){
                                layer.msg(response.body.msg, {icon:1,time:1000});
                                window.location.reload();
                            }else{
            ...
            changeStatus: function (index, name, id, status,list) {
                ...
                layer.confirm('确定要更改员工 '+name+' 人员状态吗？', {icon: 3, title:'提示'},
                    function () {
                        _this.$http.post('{{ route('employee.api_change_status') }}', {_token:"{{csrf_token()}}",id:id, status:status}).then(function (response) {
                            if (response.body.status == 'success'){
                                layer.msg(response.body.msg, {icon:1,time:1000});
            ...
            getEmployeeList: function () {
                ...
                var sv = '{{$sv}}';
                var tit = '{{$tit}}';
                this.$http.post("/book/employee/list", {_token: "{{csrf_token()}}", 'dep_id': dep_id, 'page': page, 'sv':sv, 'tit':tit}).then(function (response) {
