...
@section('content')
    <div class="recoardWrapper" v-cloak>
            ...
            <div class="recoardMenu">
                ...
                <div class="recoardBar">
                    <div class="barMark">
                        ...
                        <input  :min="1" :max="99999999" step="1" type="number" v-model="numVal">
            ...
            <div class="recoardTable">
                <table cellspacing="0" border="0" style="border-collapse:collapse;">
                    ...
                    <tr v-for="(certificate,index) in certificateTable" :key="certificate" class="contentItem">
                        ...
                        <td ref="tdContent">
                            <input type="text" class="celInput abstract" @keyup="keyup(certificate, $event)" ref="celInput" :value="certificate.zy" v-model="certificate.zy" :disabled="disableTrue" onfocus="this.select()" @keyup.enter="inputNext(certificate,index,$event)">
                        </td>
                        <td>
                            <div @click="flag && getAdd(certificate,index)" ref="kjkmMenu" class="curList">
                                <input type="text" class="celInput hiddenInput" @focus="getAdd(certificate,index)" @input="value == '' ? getAdd(certificate,index,$event) : ''" :value="certificate.codeInput" v-model="certificate.codeInput" :disabled="disableTrue" @keyup.enter="inputNext(certificate,index,$event)">
                        ...
                        <td class="colSend jf" @click="flag && total(certificate,$event)">
                         <input type="text" class="celInput bigWord entry debit" maxlength="11" v-show="certificate.lendInput" :value="certificate.val" v-model="certificate.val" @keyup.enter="inputNext(certificate,index,$event)" @blur="moneyVal(certificate,index)" :disabled="disableTrue" @input="phLend(certificate,index)">
                        ...
                        <td class="colSend df" @click="flag && loan(certificate,$event)">
                            {{--<input type="text" class="celInput bigWord credit" maxlength="11" :value="certificate.sendVal" v-model="certificate.sendVal" @blur="sendVal(certificate,index)" @focus="sendValue(certificate,index)" :disabled="disableTrue">--}}
                            <input type="text" class="celInput bigWord credit" maxlength="11" v-show="certificate.loanInput" :value="certificate.sendVal" v-model="certificate.sendVal" @blur="sendVal(certificate,index)" @keyup.enter="inputNext(certificate,index,$event)" :disabled="disableTrue"  @input="phLoan(certificate)">
        ...
        <div class="footer">
            <a href="javascript:void(0);" class="keepAdd" @click="keepAdd" v-show="keepShow">保存并新增(F12)</a>
            <a href="javascript:void(0);" class="recoardItem" @click="keepBtn" v-show="keeps">保存(Ctrl+S)</a>
            <a href="javascript:void(0);" class="recoardItem" v-show="loading">loading...</a>
            <a href="javascript:void(0);" class="recoardItem" @click= "auditeBtn" v-show="auditeShow">审核</a>
            <a href="javascript:void(0);" class="recoardDel" v-show="auditeShow" @click="del">删除</a>
            <a href="javascript:void(0);" class="recoardItem" v-show="Naudite" @click="antiAudit">反审核</a>
        </div>
    </div>
@endsection
        ...
        var vm = new Vue({
            ...
            methods: {
                ...
                getListData:function(){
                    ...
                    _this.$http.post('{{ route('voucher.api_get_voucher') }}', data).then(function (response) {
                        response = response.body;
                        if (response.status == 'success') {
                            var arr = response.items;
                            for(var i in arr){
                                arr[i].lendInput = false;
                                arr[i].lendShow = true;
                                arr[i].loanInput = false;
                                arr[i].loanShow = true;
                                arr[i].val = Number(arr[i].val).toFixed(2);
                                arr[i].sendVal = Number(arr[i].sendVal).toFixed(2);
                                if(arr[i].val == 0){
                                    arr[i].val = ''
                                }
                                if(arr[i].sendVal == 0){
                                ...
                                arr[i].jfVal = arr[i].val * 100;
                                arr[i].dfVal = arr[i].sendVal * 100;
                                if(arr[i].jfVal == 0){
                                    arr[i].jfVal = ''
                                }
                                if(arr[i].dfVal == 0){
                            ...
                            _this.period = response.period;
                            _this.newAdds = response.kuaijikemu;
                            _this.numVal = response.data.voucher_num;
                            _this.dateVal = response.data.voucher_date;
                            _this.certificateTable = arr;
                            /*_this.valTotal = response.data.total_debit_money;
                            _this.sendTotal = response.data.total_credit_money;*/
                            _this.dxMoney = response.data.total_cn;
                            _this.voucher_source = response.data.voucher_source;
                            _this.voucher_date = response.data.voucher_date;
                            _this.create_by = response.data.creator_name;
                            _this.create_at = response.data.created_at;
                            _this.review_by = response.data.auditor_name;
                            _this.review_at = '';
                            ...
                            if(response.data.audit_status == '1'){
                                ...
                                //借方移除点击事件
                                _this.flag = false;
                                _this.review_at = response.data.updated_at;
                            }
                ...
                condition:function(){
                    ...
                    var param = {
                        ...
                        'voucher_date':this.voucher_date,'voucher_source':this.voucher_source,'total_debit_money':this.valTotal,
                ...
                keepBtn:function(){
                    ...
                    _this.$http.post('{{route('voucher.make')}}',params).then(function(response){
                        ...
                        setTimeout(function(){_this.loading = false;_this.keeps = true;}, 1200);
                    })
                },
                /*----审核按钮----*/
                auditeBtn:function(){
                    /*------条件符合的情况下传给后端----*/
                    var _this = this;
                    var params = {'_token': '{{ csrf_token() }}', 'id': [_this.vid],'audit_status':'1'};
                    //console.log(params);
                ...
                antiAudit:function(){
                    ...
                    var params = {'_token': '{{ csrf_token() }}', 'id': [_this.vid],'audit_status':'0'};
                    _this.$http.post('{{route('voucher.audit')}}',params).then(function(response){
                        response = response.body;
                        //console.log(response);
                        if(response.status == 1){
                ...
                del:function(){
                    var  _this = this;
                    var params = {'_token': '{{ csrf_token()  }}', 'id': [_this.vid]};
                    layer.open({
                        type: 1,
                        title: '信息',
                        skin: 'delAleart',
